...
class LifecycleNodesListWatcher(WatcherBase):
    ...
    def run(self) -> None:
        while not self._exit_event.is_set():
            ...
            node_names = r2CycleApi.get_node_names(node=self.node.node, include_hidden_nodes=True)

            states = r2CycleApi.call_get_states(node=self.node.node, node_names=[name.full_name for name in node_names])
